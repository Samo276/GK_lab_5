...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
	...
	//--------------------------------------------------------------------------------------------------------------
	private void draw_stozek(GL2 gl2) {
		gl2.glBegin (gl2.GL_TRIANGLES);
		gl2.glColor3f (1,1,0);
		
		int promien = 3;
		int il_katow = 17;
		float wysokosc_stozka = 3;
		
		for(int i = 1; i<=il_katow; i++) 
		{
		 draw_trojkat(promien,gl2,i,0);								//podstawa
		 draw_trojkat_sciany(promien,gl2,i, wysokosc_stozka);		//sciany
		}
		gl2.glEnd ();
	}
	private void draw_trojkat(int promien, GL2 gl2, int i, float wysokosc) {
		
		gl2.glVertex3f(0,(float)(promien*Math.cos(2*i*Math.PI /17)), (float)(promien*Math.sin(2*i*Math.PI /17)));
		gl2.glVertex3f(0,(float)(promien*Math.cos(2*(i+1)*Math.PI /17)), (float)(promien*Math.sin(2*(i+1)*Math.PI /17)));
		gl2.glVertex3f(wysokosc,0, 0);
	}
	private void draw_trojkat_sciany(int promien, GL2 gl2, int i, float wysokosc) {
	...
	//---------------------------------------------------------------------------------------------------------------
	private void draw_spirale(GL2 gl2) {
